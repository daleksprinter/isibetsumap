...
function spot_select(){
    ...
    for(var key in list){
     ...
     for (var i = 0; i < findli_child; i++){
                 ...
                 icon.setAttribute('icon','circle-o');
                 icon.setAttribute('size','40px');
                 icon.setAttribute('onclick','check(this.previousElementSibling)')
                 findLi[i].appendChild(icon);
...
function check(elm){
    ...
    if(String(elm.id) in list){
        ...
        elm.nextElementSibling.setAttribute('icon','circle-o');
    }else{
        ...
        elm.nextElementSibling.setAttribute('icon','check-circle');
    }
}
...
//カルーセルアイテムのスポット説明部分を作成する関数。クラスの割り当てられたテキストを返すのでそれをそのまま親要素に追加する。
